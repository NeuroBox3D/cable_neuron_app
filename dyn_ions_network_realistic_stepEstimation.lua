...
-- choice of grid
gridName = util.GetParam("-grid", "Skalierung_5000_secs.ugx")
...
-- file handling
fileName = util.GetParam("-outName", "Network_dyn_ions")
...
-- Flux of HH-NaDyns = na/kpump-Na-flux with pump max parameter in v_eq
-- Flux of Leak of K = na/kpump-K-flux + HH-K-flux in v_eq 
----------------------------------------------------------------------------------------------------------
if withIons == true then
...
if (generateVTKoutput) then 
	...
	out:print_subset(fileName.."_soma", u, 2, 0, time)
end
...
while endTime-time > 0.001*curr_dt do
	...
	if (generateVTKoutput) then
		if math.abs(time/pstep - math.floor(time/pstep+0.5)) < 1e-5 then 
			...
			out:print_subset(fileName.."_soma", u, 2, math.floor(time/pstep+0.5), time)
		end
	end
...
if (generateVTKoutput) then
	...
	out:write_time_pvd(fileName.."_soma", u)
end
