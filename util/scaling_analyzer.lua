...
inFiles = {"profile0256.log", "profile0512.log", "profile1024.log", "profile20248.log"}


--	num digits behind comma
numDigitsBehindComma = 2
minimumTime = 0.1 --- set this to 0.1 to display only times with > 0.1 s
			...
			--	get entries for each line through pattern matching
				local 	spaces, name, hits, selfTime, selfUnit, selfPerc,
						...
						= string.match(line, "(%s*)(<*%a[_%.%,():%w]+>*)%s+([%.%,%deE+]+)%s+" -- leading spaces, name and hits
								   .."([%.%,%deE+-]+)%s+(%a+)%s+([%,%.%d]+)%s*%%%s+"	  -- self:  time, unit, percent
								   .."([%.%,%deE+-]+)%s+(%a+)%s+([%,%.%d]+)%s*%%")		  -- total: time, unit, percent
				...
				--	add the timing to the fileTimings list
					...
					entry.spaces = spaces
					entry.name = name
					totalTime = string.gsub(totalTime, ",", ".")	-- replace possible "," by "." in time format
...
--	If the profilings contain a "lua_pcall" entry, we will subtract all timings
--	from direct child entries of "lua_pcall" and store the result in an additional
--	entry named "unknown"
for _, timing in ipairs(timings) do
	local lua_pcallEntry = nil
	local childTimingSum = 0
	local highestIndex = 0
	local spaces = "  "
	for ind, entry in ipairs(timing) do
		if entry.name == "lua_pcall" then
			lua_pcallEntry = entry
			spaces = entry.spaces.." "
		elseif lua_pcallEntry ~=nil and entry.spaces == spaces then
...
--	if a "main" entry was found, we will now add a new child entry called "unknown"
	if lua_pcallEntry ~= nil then
		local entry = {}
		entry.spaces = spaces
		entry.name = "unknown"
		entry.time = lua_pcallEntry.time * timeFactors[lua_pcallEntry.timeUnit] - childTimingSum
		entry.timeUnit = "s"
	...
	--	iterate over all timings and calculate speedups
		...
		-- store the times in seconds.
			...
			if curEntry1 == nil then
				...
				tStr = string.format("%."..numDigitsBehindComma.."f", curEntry1.time) .. " " .. curEntry1.timeUnit
			end
